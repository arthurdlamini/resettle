...
function Graves() {
			...
			return (
				<>
					...
					<FaEye
							className='mx-2 action-icon-edit'
							onClick={() => {
								navigate(`/graves/view/${params.row._id}`);
							}}
						/>
				</>
			);
	...
	const deleteGraveBtn = (gravee) => {
		dispatch(loading(true));
		api(user)
			.delete(`/graves/${gravee._id}`)
			.then((response) => {
				if (response.data) {
					dispatch(loading(false));
					notifySuccess('Grave Deleted');
					handleClose();
					getGraves();
				}
				dispatch(loading(false));
		...
		<div className='pt-2 px-3'>
			...
			<Modal
				...
				<Modal.Header closeButton>
					<Modal.Title>Confirm Grave Deletion</Modal.Title>
				</Modal.Header>
					...
					<div className='form-group'>
						...
						<div className='col-md-12 mt-2'>
							<input
								...
								name='catdesc'
								type='text'
								placeholder={selectedGrave.homesteadId?.homesteadHead?.name}
